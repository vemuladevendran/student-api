...
const getCircular = async (req, res) => {
  try {
    const result = await Circular.find({ isDeleted: false }).sort({ createdAt : 'descending'});;
...
const getCircularByBranch = async (req, res) => {
  try {
    ...
    const result = await Circular.find({
      isDeleted: false,
      $or: [{ circularFor: "all" }, { circularFor: branch }],
...
const deleteCircular = async (req, res) => {
  try {
    // cheaking user
    const results = await Circular.findOneAndUpdate(
      { id: req.params.id },
      { isDeleted: true }
    );
